 ...
 * @since 1.7.0
 */
class Saasland_appart_hero extends Widget_Base {
    ...
    protected function _register_controls() {
        ...
        $buttons->add_control(
            'bg_color2', [
                ...
                'type' => Controls_Manager::COLOR,
                'selectors' => array(
                    '{{WRAPPER}} {{CURRENT_ITEM}}' => 'background-image: -webkit-linear-gradient( 0deg, {{bg_color.VALUE}} 0%, {{VALUE}} 100%)',
                ),
                'condition' => [
                    'btn_style' => ['gradient']
                ]
        ...
        $buttons2 = new \Elementor\Repeater();

        $buttons2->add_control(
            'label', [
        ...
        $buttons2->start_controls_tabs(
            'style_tabs'
        );
        /// Normal Button Style
        $buttons2->start_controls_tab(
            'style_normal_btn',
            [
                'label' => __( 'Normal', 'plugin-name' ),
            ]
