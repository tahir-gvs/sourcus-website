...
$language = (function_exists('icl_object_id') ? ICL_LANGUAGE_CODE : "");
$css_global = "";
...
if (sb_get($sb_config,"privacy-active")) {
    $attr .= ' data-privacy="' . sb_get($sb_config,"privacy-msg") . '"';
    if (sb_get($sb_config,"privacy-btn-approve") != "") {
        $attr .= ' data-privacy-btn-approve="' . sb_get($sb_config,"privacy-btn-approve") . '"';
    }
    if (sb_get($sb_config,"privacy-btn-decline") != "") {
        $attr .= ' data-privacy-btn-decline="' . sb_get($sb_config,"privacy-btn-decline") . '"';
    }
    if (sb_get($sb_config,"privacy-msg-decline") != "") {
        $attr .= ' data-privacy-msg-decline="' . sb_get($sb_config,"privacy-msg-decline") . '"';
    }
}
...
<div id="sb-main" class="sb-main <?php echo $css_global ?>" <?php echo $attr ?> style="display:none" data-language="<?php echo $language ?>">
    <?php if ($atts["type"] == "board") { ?>
    <div class="sb-cnt-global sb-cnt <?php echo $css_board ?>" style="<?php echo $style_board ?>">
        <div class="sb-header">
            <div class="sb-title">
                <img class="sb-header-img" src="<?php echo ((isset($user_arr["img"])) ? $user_arr["img"] : SB_PLUGIN_URL . "/media/user-2.jpg") ?>" alt="" />
                <span>
                    <?php echo $user_name ?>
                </span>
            </div>
            ...
            <div class="sb-chat-header" <?php if (sb_get($sb_config, "chat-header-img") != "" && strpos(sb_get($sb_config, "chat-header-img"),"plus-large.png") == false) echo 'style="background-image:url(' . $sb_config[ "chat-header-img"] . ')"' ?>>
                ...
                <?php
                  if ($chat_header == "brand") {
                      echo '<div class="sb-header-brand"><img src="' . sb_get($sb_config,"chat-brand-img") . '" alt="" /></div>';
                  } else {
                      if (sb_get($sb_config,"chat-avatars") && isset($agents_arr)) {
                          ...
                          if ($c > 3) $c = 3;
                          for ($i = 0; $i < $c; $i++)  {
                              $html .= '<div><img src="' . $agents_arr[$i]["img"] . '" alt="" /><span>' . $agents_arr[$i]["username"] . '</span></div>';
                          }
                          if ($html != "") {
